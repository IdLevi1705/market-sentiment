// src/pages/api/stocks/gainers.ts
...
// List of common stocks to check
const COMMON_STOCKS = [
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Sort by percent change (descending) to get top gainers
    stocksData.sort((a, b) => b.changePercent - a.changePercent);
    ...
    if (gainers.length === 0) {
      return res.status(404).json({ error: "No gaining stocks found" });
    }
